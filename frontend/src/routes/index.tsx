...
function DashboardPage() {
  ...
  const average = records.length ? records.reduce((total, record) => total + record.confidence, 0) / records.length : 0;
  ...
  return <div className="space-y-6"><PageHeader eyebrow="Overview" title="Political Emotion Intelligence" description="Review analyses saved in this browser." />
    <div className="grid gap-4 sm:grid-cols-3"><StatCard label="Analyses" value={records.length} icon={FileText} /><StatCard label="Average confidence" value={`${average.toFixed(1)}%`} icon={Target} /><StatCard label="Most detected" value={mostDetected} icon={BarChart3} /></div>
