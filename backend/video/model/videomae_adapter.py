...
class VideoMAEAdapter(VideoModelInterface):
    ...
    current video API. Internally, this adapter uses the pretrained ViT-Tiny
    ...
    def _get_default_model_path(self) -> Path:
        ...
        if not cache_root.exists():
            raise FileNotFoundError(
                "The pretrained ViT-Tiny model has not been downloaded. "
                "Run the Hugging Face download command first."
            )
        ...
        if not candidates:
            raise FileNotFoundError(
                "Could not find emotion_vit_tiny.torchscript.pt "
    ...
    def load_model(
        ...
        **kwargs: Any,
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                f"Emotion model file does not exist: {model_path}"
            )
    ...
    def _aggregate_predictions(
        ...
        if len(predictions) == 0:
            raise ValueError(
                "At least one frame prediction is required."
            )
        ...
        if total <= 0.0:
            raise RuntimeError(
                "Emotion model produced zero probability mass."
            )
    ...
    def predict(
        ...
        if (
            ...
            raise RuntimeError(
                "Emotion model is not loaded. "
                "Call load_model() before calling predict()."
            )
        ...
        return {
            ...
            "message": (
                ...
                "The result represents visible facial expression "
                "and may not fully represent the underlying emotion."
            ),
    ...
    def predict_batch(
        ...
        if (
            ...
            raise RuntimeError(
                "Emotion model is not loaded. "
                "Call load_model() before calling predict_batch()."
            )

        batch = list(frames_batch)

        if len(batch) == 0:
            raise ValueError(
                "At least one video frame sequence is required."
            )
